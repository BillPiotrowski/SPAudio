...
typealias AudioSignal = (_ frequency: Float, _ time: Float) -> Float
...
struct Oscillator {
    ...
    static let sine: AudioSignal = { frequency, time in
        return Oscillator.amplitude * sin(2.0 * Float.pi * frequency * time)
    }
    
    static let triangle: AudioSignal = { frequency, time in
